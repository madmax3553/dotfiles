return {
				...
				sections = {
					{
						...
						{
							section = 'terminal',
							cmd = 'chafa ~/Pictures/nvimlogos/tanya.gif --format symbols --symbols vhalf --size 35x50 --align hcenter --center true; sleep .1',
							height = 35,
							width = 50,
							padding = 1,
						},
		...
		keys = {
			...
			},
			{
				...
				function()
					...
					vim.defer_fn(function() vim.cmd('stopinsert') end, 50)
				end,
				desc = 'Diagnostics',
			},
			{
			...
			{
				'<leader>sL',
				function() require('snacks').persistence.load({ last = true }) end,
	...
	},
	{
		...
		opts = {
			enabled = false,
			dir = vim.fn.stdpath('state') .. '/snacks/persistance',
			options = { 'buffers', 'curdir', 'tabpages', 'winszie' },
		},
	},
	-- init = function()
